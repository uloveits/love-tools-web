...
export default class ColumnClassifyPage extends React.Component {
  static propTypes = {
    location: P.any,
    history: P.any,
    actions: P.any,
  ...
  getTblList = async () => {
    ...
    let res = await columnApi.classifyList(params);
  ...
  delete = async(data) => {
    ...
    let res = await columnApi.deleteClassify({id:data.id})
    if(res.code === '200') {
      message.success('删除成功！')
      await this.getTblList();
    }
  }
  ...
  onOK = async () => {
    ...
    if(modalTitle === '新增'){
      ...
      let res = await columnApi.updateClassify(param);
      if(res.code === '200') {
